...
A simple, user-friendly web application featuring a voice-activated chatbot powered by the Groq API. The bot is designed to answer questions, providing a seamless voice-in, voice-out user experience.
...
-   **AI-Powered Responses:** Integrates with the  Groq API to generate intelligent and context-aware answers.
...
-   **Secure API Key Handling:** Employs a serverless function on Vercel to protect the Groq API key.
...
3.  **Secure Backend Processing:** The Vercel function securely accesses the `Groq API` from its environment variables and sends a request to the Groq API.
4.  **AI Generation:** The Groq API generates a response which is sent back to the serverless function, which immediately relays it to the frontend.
...
* **"Wake Word" Activation:** Implement a "Hey Groq" wake word to activate the bot without needing to click a button.
* **Multilingual Support:** Leverage the Groq API's multilingual capabilities to allow users to converse in different languages.
